...
export async function createRewardWarmindo(
  ...
  const rawGambar = (formData.get("gambar") as string) || null;
  let finalGambarUrl: string | null = null;
  if (rawGambar && rawGambar.trim() !== "") {
    if (rawGambar.startsWith("data:image/")) {
      try {
        const uuid = randomUUID();
        finalGambarUrl = await uploadImageToR2(
          rawGambar,
          "reward-warmindo",
          `reward-${uuid}`,
        );
      } catch (err) {
        console.error("Gagal upload gambar reward ke R2:", err);
      }
    } else {
      finalGambarUrl = rawGambar.trim();
    }
  }
  ...
  try {
    ...
    revalidatePath("/reward-warmindo");
