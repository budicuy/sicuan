...
export async function updateSetorSampahStatus(
  ...
  try {
    ...
    // Kirim notifikasi email ke Warmindo jika status diubah menjadi diverifikasi (ditugaskan ke ekspedisi)
    if (
      status === "diverifikasi" &&
      roleTarget === "warmindo" &&
      depositor?.email
      ...
      try {
        await sendAssignmentNotifToWarmindo({
          warmindoEmail: depositor.email,
          warmindoName: depositor.name,
    ...
    if (userWithEmail?.email) {
      ...
      after(async () => {
        try {
          ...
          console.error(
            "Gagal kirim email notif admin (createSetorSampah warmindo):",
            err,
          );
        }
      });
      after(async () => {
        try {
          ...
          console.error(
            "Gagal kirim email tanda terima ke depositor (createSetorSampah warmindo):",
            err,
          );
        }
      });
