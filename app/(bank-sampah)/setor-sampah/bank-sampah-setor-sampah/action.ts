...
export async function updateSetorSampahStatus(
  ...
  try {
    ...
    // Update nasabah balance directly (skip credit for warmindo since it's accumulated monthly)
    const _isWarmindo = depositor?.role === "warmindo";
    await db
      .update(nasabah)
      .set({
...
export async function submitSetorSampah(
  ...
  try {
    ...
    if (!isPending) {
      // Update nasabah balance directly (skip credit for warmindo since it's accumulated monthly)
      const _isWarmindo = user.role === "warmindo";
      await db
        .update(nasabah)
        .set({
          poin: sql`${nasabah.poin} + ${totalPoin}`,
...
export async function bankSampahTerimaSetoran(
  ...
  if (Number.isNaN(beratAktual) || beratAktual <= 0) {
    return {
      success: false,
      message: "Berat aktual harus lebih dari 0 kg.",
    };
  }

  if (!fotoTimbanganBase64 || fotoTimbanganBase64.trim() === "") {
    return {
      success: false,
      message: "Foto timbangan verifikasi wajib dilampirkan.",
    };
  }
  ...
  try {
    ...
    const isValidStatus =
      ...
      (item.status === "pending" && item.metodeSetor === "langsung");

    if (!isValidStatus) {
      return {
        success: false,
        message:
          "Setoran ini belum dalam status yang siap diverifikasi — harap pastikan kurir telah mengantar atau metode adalah datang langsung.",
      };
    }

    // Upload foto timbangan verifikasi ke R2
    ...
    try {
      ...
      console.error("Gagal mengupload foto timbangan verifikasi:", err);
      return {
        success: false,
        message: "Gagal mengupload foto timbangan verifikasi ke storage.",
      };
    }
    ...
    // Kirim notifikasi email status update ke nasabah Warmindo secara asinkron di latar belakang menggunakan Next.js after()
    if (!isPending && depositor?.email) {
      const email = depositor.email;
      after(async () => {
        try {
          ...
          console.error(
            "Gagal mengirim email status update ke nasabah Warmindo:",
            err,
          );
        }
      });
