...
export async function handoverSetorSampahToEkspedisi(
  ...
  try {
    ...
    // Kirim email notifikasi ke Bank Sampah secara asinkron di latar belakang menggunakan Next.js after()
    after(async () => {
      try {
...
export async function createSetorSampah(
  ...
  if (!isWarmindo && !fotoTimbanganBase64) {
    return {
      success: false,
  ...
  if (!isWarmindo && fotoBuktiBase64List.length < 1) {
    return {
      success: false,
  ...
  if (!isWarmindo && fotoBuktiBase64List.length > 3) {
    return {
      success: false,
  ...
  if (isWarmindo && !bankSampahId) {
    return {
      success: false,
      errors: { _form: ["Tujuan Bank Sampah wajib dipilih."] },
    };
  }
  ...
  // Upload foto bukti tambahan ke R2 (hanya jika bukan Warmindo)
  const fotoBuktiUrls: string[] = [];
  if (!isWarmindo) {
    try {
  ...
  try {
    // Untuk warmindo: hormati pilihan metode (ekspedisi/langsung)
    // Untuk non-warmindo: selalu langsung
    const effectiveMetode = isWarmindo
      ? (metodeSetor as "langsung" | "ekspedisi")
      : "langsung";
    const isEkspedisi = effectiveMetode === "ekspedisi";
    const isPending =
      isEkspedisi ||
    ...
    const baseValues = {
      ...
      status: (isPending ? "pending" : "diterima") as
        ...
        | "ditolak",
      kategoriNasabah: user.role as "konsumen" | "warmindo" | "bank-sampah",
      metodeSetor: effectiveMetode,
      bankSampahId: isWarmindo ? bankSampahId : null,
    };
    ...
    after(async () => {
      ...
      try {
        await sendSetoranNotifToAdmins({
          ...
          fotoTimbanganBase64: fotoTimbanganBase64 || "",
          fotoBuktiBase64List: fotoBuktiBase64List || [],
        });
      } catch (err) {
